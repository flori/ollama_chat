describe OllamaChat::WebSearching do
  ...
  describe '#search_web' do
    it 'returns nil and warns for an unimplemented engine' do
      allow(chat).to receive(:search_engine).and_return('nonexistent')
    ...
    it 'delegates to the engine method and collects links' do
      allow(chat).to receive(:search_engine).and_return('duckduckgo')
  ...
  describe '#web' do
    ...
    it 'returns :next when no search results are found' do
      allow(chat).to receive(:search_web).and_return(nil)
      expect(chat.web('1', 'ruby')).to be(:next)
    end

    context 'when policy is embedding and embedding is on' do
      before do
        allow(chat.document_policy).to receive(:selected).
          and_return('embedding')
        allow(chat.instance_variable_get(:@embedding)).
          to receive(:on?).and_return(true)
      end

      it 'embeds each source and interpolates the web_embed prompt' do
        allow(chat).to receive(:prompt).with(:web_embed).
          and_return(double('prompt', to_s: 'Search for: %{query}'))
        allow(chat).to receive(:search_web).and_return(urls)
        allow(chat).to receive(:fetch_source) do |_url, &block|
          block&.call(nil)
        end
    ...
    context 'when policy is summarizing' do
      before do
        allow(chat.document_policy).to receive(:selected).
          and_return('summarizing')
      end

      it 'summarizes each URL and interpolates the prompt' do
        allow(chat).to receive(:prompt).with(:web_import).
          and_return(double('prompt', to_s: 'Q: %{query} R: %{results}'))
        allow(chat).to receive(:search_web).and_return(urls)
        allow(chat).to receive(:summarize).and_return('summary text')
    ...
    context 'when policy is importing (default)' do
      before do
        allow(chat.document_policy).to receive(:selected).
          and_return('importing')
      end

      it 'imports each URL and interpolates the prompt' do
        allow(chat).to receive(:prompt).with(:web_summarize).
          and_return(double('prompt', to_s: 'Q: %{query} R: %{results}'))
        allow(chat).to receive(:search_web).and_return(urls)
        allow(chat).to receive(:import).and_return('imported text')
        result = chat.web('2', 'ruby')
